        ...
        '{{#data}}<div class="panel-body">',
            '<div class="card-alert">',
                ...
                '<div class="alert-body">',
                    ...
                    '{{^isDeleteOrOverTimePlan}}<a href="examForResult.html?er_id={{er_id}}&user_id={{user_id}}&restart=true">查看考试结果</a>{{/isDeleteOrOverTimePlan}}',                   
                    '</div>{{/isExamed}}',
                    '<div class="item btn">',       
                        '<span>状态: {{show_status}}</span>',                   
                        '{{^isDeleteOrOverTimePlan}}<a href="test.html?ep_id={{ep_id}}&user_id={{user_id}}">{{exam_btn_txt}}</a>{{/isDeleteOrOverTimePlan}}',                   
                    '</div>',
                '</div>',
            '</div>',
        '</div>{{/data}}',
