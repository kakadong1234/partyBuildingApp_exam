...
function getExamList() {
    //TODO
    $.get('https://api.lpszzb.gov.cn/exam/result?user_id=' + user_id, function(data){
        console.log(data);
        ...
        '{{#data}}<div class="panel-body">',
            '<div class="card-alert">',
                ...
                '<div class="alert-body">',
                    ...
                    '{{^isDeleteOrOverTimePlan}}<a href="examForResult.html?er_id={{er_id}}&user_id={{user_id}}&isFrom=myList&restart=true&version='+ version +'">查看考试结果</a>{{/isDeleteOrOverTimePlan}}',                   
                    '</div>{{/isExamed}}',
                    '<div class="item btn">',       
                        '<span>状态: {{show_status}}</span>',                   
                        '{{^isDeleteOrOverTimePlan}}<a href="test.html?ep_id={{ep_id}}&user_id={{user_id}}&isFrom=myList&version='+ version +'">{{exam_btn_txt}}</a>{{/isDeleteOrOverTimePlan}}',                   
                    '</div>',
                '</div>',
            '</div>',
        '</div>{{/data}}',
