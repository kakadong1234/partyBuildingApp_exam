...
function getExamData() {
    //TODO
    $.post('https://api.lpszzb.gov.cn/exam/result?ep_id=' + ep_id + '&user_id=' + user_id, function(data){
        console.log(data);
        er_id = data.er_id;
...
function finishExam(){
    ...
    if(submitStatus === "未交卷" || submitStatus === "交卷失败"){
        ...
        $.ajax({  
            type: "post",  
            url: "https://api.lpszzb.gov.cn/exam/test",  
...
function sendUserAnswer(data) {
    ...
    $.ajax({  
        type: "post",  
        url: "https://api.lpszzb.gov.cn/exam/plan",  
        contentType : "application/x-www-form-urlencoded; charset=UTF-8",  
        data: $.param({
            user_id: user_id,
